...
class KAKAOBLINDRECRUITMENT2021Lv1 {
...
*/
    ...
    public static String solution(String new_id) {
        ...
        // 2단계 new_id에서 알파벳 소문자, 숫자, 빼기(-), 밑줄(_), 마침표(.)를 제외한 모든 문자를 제거합니다.
        if (!Pattern.matches("[a-z0-9._-]*$", answer)) {
            StringBuilder str = new StringBuilder();
            for (Character c : answer.toCharArray()) {
                if (Pattern.matches("[a-z0-9._-]*$", c.toString())) {
                    str.append(c);
                }
            }
            answer = str.toString();
        ...
        // 4단계 new_id에서 마침표(.)가 처음이나 끝에 위치한다면 제거합니다.
        if (answer.startsWith(".") && !answer.isEmpty()) {
            answer = answer.replaceFirst(".", "");
        }

        System.out.println("### " + answer);

        if (answer.endsWith(".") && !answer.isEmpty()) {
            answer = answer.replaceFirst(".$","");
        }
    ...
    public static void main(String[] args) {
        ...
        String new_id6 = " lskjdfia.skd2#%#$&m";
        ...
        System.out.println(solution(new_id6));

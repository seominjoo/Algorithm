...
class DevMatching웹백엔드개발2021Lv1 {
...
*/
    ...
    public static int[] solution(int[] lottos, int[] win_nums) {
        ...
        // 맞춘 수
        long equalsCount = Arrays.stream(lottos).filter(lotto -> winNumList.contains(lotto)).count();
        // 0 갯수
        long zeroCount = Arrays.stream(lottos).filter(lotto -> lotto == 0).count();

        // 최고 순위
        int max = Math.toIntExact(7 - (zeroCount + equalsCount));
        // 최저 순위
        int min = Math.toIntExact(7 - equalsCount);
    ...
    public static void main(String[] args) {
        ...
        int[] lottos4 = {1, 2, 5, 6, 7, 8};
        int[] win_nums4 = {20, 9, 3, 45, 4, 35};
        ...
        System.out.println(Arrays.toString(solution(lottos4, win_nums4)));
    }
}